 ...
 * GET - https<nolink>://*API_ID*.execute-api.*REGION*.amazonaws.com/*STAGE*/node/{id}/update
 ...
 Notice that the TaScaaS package includes a Python script (*worker/worker.py*) that can be used to communicate with this API and simplify the integration with the user application. See the readme in the *examples* folder to learn how to use it.
